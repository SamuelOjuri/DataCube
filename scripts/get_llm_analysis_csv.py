...
def build_segment_stats(db: SupabaseClient, project: Dict[str, Any]) -> SegmentStatistics:
    ...
    backoff = 0  # most specific

    # Fallback to (type, category) for this helper
    if not cm:
...
def fetch_segment_df(db: SupabaseClient, key: Dict[str, Any], lookback_days: Optional[int] = None):
    """
    Return (segment_df, segment_keys_used, backoff_tier).

    NOTE:
    - backoff_tier returned here is internal zero-based tier (0..5) for model computation.
    - reporting converts this to priority tier (1..6) with labels.
    """
    candidates = [
    ...
    for tier, (fields, min_required) in enumerate(candidates):
    ...
    return pd.DataFrame(), [], len(candidates) - 1
...
def write_csv(results: List[Dict[str, Any]], csv_file: Path) -> None:
    headers = [
        ...
        'segment_keys', 'segment_sample_size',
        'segment_backoff_tier', 'segment_backoff_tier_internal', 'segment_backoff_label', 'segment_backoff_min_n',
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        ...
        for r in results:
            ...
            internal_tier = seg.get('backoff_tier_internal', 5)
            fallback_meta = get_backoff_priority_metadata(internal_tier)
            row = {
                ...
                'segment_backoff_tier': seg.get('backoff_tier', fallback_meta['priority_tier']),
                'segment_backoff_tier_internal': seg.get('backoff_tier_internal', internal_tier),
                'segment_backoff_label': seg.get('backoff_label', fallback_meta['label']),
                'segment_backoff_min_n': seg.get('backoff_min_n', fallback_meta['min_n']),
        ...
        try:
            ...
            res = {
                ...
                'segment': {
                    ...
                    'backoff_tier': seg_stats.backoff_priority_tier if seg_stats else 6,         # reporting tier
                    'backoff_tier_internal': seg_stats.backoff_tier if seg_stats else 5,         # internal tier
                    'backoff_label': seg_stats.backoff_priority_label if seg_stats else 'Global median',
                    'backoff_min_n': seg_stats.backoff_min_n if seg_stats else 1,
