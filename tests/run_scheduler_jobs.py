...
async def time_recent_rehydrate():
    async with _timer("Recent rehydrate (Supabase upserts)"):
...
async def time_monday_sync():
    dry_run = os.getenv("MONDAY_SYNC_DRY_RUN", "").lower() in {"1", "true", "yes"}
    async with _timer(f"Monday sync (dry_run={dry_run})"):
        await _scheduled_monday_sync()

async def time_all():
    # Optional: force Monday sync into dry-run mode to avoid pushing updates
    os.environ.setdefault("MONDAY_SYNC_DRY_RUN", "false")

    await time_delta_rehydrate()
    await time_recent_rehydrate()
    await time_llm_backfill()
    await time_monday_sync()
